...
import touchpads, keypad, display, wifi, audio, time, system, appconfig
from umqtt.simple import MQTTClient

# Get the settings from the settings menu
settings = appconfig.get('mqtt_button', {'MQTT_server_ip': "Your IP", 
                                         'MQTT_device_name': "/cz2020",
                                         'MQTT_discovery_prefix': 'homeassistant' })
SERVER_IP = settings['MQTT_server_ip']
DEVICE_NAME = settings['MQTT_device_name']+"/"
PREFIX = settings['MQTT_discovery_prefix']+"/" # todo fix that the all / are correct
DEVICE_CONFIG = {
    "identifiers": "5ccf7f0f4ff3",
    "name": DEVICE_NAME,
